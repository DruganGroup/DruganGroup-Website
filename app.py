...
def emergency_restore():
    ...
    admin_email = "admin@drugangroup.co.uk"  # <--- PUT YOUR ADMIN EMAIL HERE
    new_password = "AdminPassword123!"     # <--- This will be your new password
    ...
    try:
        ...
        if user:
            ...
            cur.execute("""
                UPDATE users 
                SET password_hash = %s, role = 'SuperAdmin' 
                WHERE email = %s
            """, (hashed_pw, admin_email))
            action = "UPDATED"
        else:
            # CREATE new admin (if it was accidentally deleted)
            cur.execute("""
                INSERT INTO users (email, username, password_hash, role, name, company_id)
                VALUES (%s, 'SuperAdmin', %s, 'SuperAdmin', 'Super Admin', NULL)
            """, (admin_email, hashed_pw))
            action = "CREATED"
            
        conn.commit()
        return f"<h1>Success: Admin {action}</h1><p>Email: {admin_email}</p><p>Password: {new_password}</p><a href='/login'>Login Now</a>"

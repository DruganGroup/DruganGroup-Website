...
# =========================================================
# 1. NEW QUOTE (With YOUR Auto-Pricing & Lead Logic)
# =========================================================
...
def new_quote():
    ...
    if request.method == 'POST':
        try:
            ...
            # --- 1. INSERT AUTO-LABOR LINE ITEM (RESTORED) ---
...
def view_quote(quote_id):
    ...
    return render_template('office/view_quote_dashboard.html', 
                           quote=quote, currency_symbol=currency,
                           brand_color=config['color'], logo_url=config['logo'])

# =========================================================
# 3. CONVERT TO JOB (The Only Change: Sync Materials)
# =========================================================
...
def convert_to_job(quote_id):
    ...
    try:
        ...
        desc = f"{title} - {q_desc}" if title and q_desc else (title or f"Work from Quote {q_ref}")
        ...
        # 3. SYNC MATERIALS (THIS IS THE FIX)
        # We copy items from Quote Items -> Job Materials so the engineer sees them
        cur.execute("""
        ...
        conn.commit()
        flash(f"✅ Job {job_ref} Created! Materials synced for engineer.", "success")
...
def email_quote(quote_id):
    ...
    # Fetch Quote (with billing address)
    cur.execute("""
    ...
    ref, client_name, client_email, title, desc, q_date, total_val, client_addr = q
    ...
    if config.get('logo') and config['logo'].startswith('/'):
        ...
        if os.path.exists(local_path): config['logo'] = f"file://{local_path}"
    ...
    context = {
        'invoice': {
            'ref': ref, 'date': formatted_date,
            'job_title': title, 'job_description': desc,
            'total': total_val, 'subtotal': total_val, 'tax': 0.0,
            'client_name': client_name, 'client_address': client_addr, 'client_email': client_email,
            'currency_symbol': settings.get('currency_symbol', '£')
        }, 
        'items': items, 'settings': settings, 'config': config, 'is_quote': True
    }
...
# =========================================================
# 5. CONVERT TO INVOICE (Restored Dynamic Days)
# =========================================================

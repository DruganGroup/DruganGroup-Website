...
def enable_portal(client_id):
  ...
  # --- SYSTEM TOOL: DATABASE MIGRATION (Run Once) ---
...
def system_upgrade():
    ...
    try:
        # 1. FIX PROPERTIES TABLE (Add 'type' column)
        try:
            cur.execute("ALTER TABLE properties ADD COLUMN IF NOT EXISTS type VARCHAR(50) DEFAULT 'Property'")
            conn.commit()
            log.append("✅ Success: Added 'type' column to Properties table.")
        except Exception as e:
            conn.rollback()
            log.append(f"ℹ️ Note on Properties: {e}")

        # 2. CREATE SERVICE REQUESTS TABLE (If missing)
        # We need this for the "Report Issue" feature to work later
        try:
            cur.execute("""
            ...
            """)
            conn.commit()
            log.append("✅ Success: Verified 'service_requests' table exists.")
        except Exception as e:
            conn.rollback()
            log.append(f"❌ Error creating Service Requests table: {e}")

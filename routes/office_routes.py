    ...
    if request.method == 'POST':
        try:
            ...
            if client_mode == 'existing': 
                ...
                """, (comp_id, request.form.get('new_client_name'), request.form.get('new_client_email'), request.form.get('new_client_address')))
...
def view_quote(quote_id):
    if session.get('role') not in ['Admin', 'SuperAdmin', 'Office', 'Site Manager']: 
        return redirect(url_for('auth.login'))
    ...
    cur.execute("""
        SELECT q.id, c.name, c.address, c.email, q.reference, q.date, q.status, q.subtotal, q.tax, q.total, q.notes 
        FROM quotes q 
...
def convert_to_invoice(quote_id):
    ...
    conn.commit()
    conn.close()
    flash(f"✅ Invoice {new_inv_ref} Created.")
...
def convert_quote_to_job():
    ...
    try:
        ...
        # Using 'start_date' and 'description' (Fix for schema mismatch)
        cur.execute("""
            INSERT INTO jobs (company_id, staff_id, client_id, start_date, description, status, ref) 
            VALUES (%s, %s, %s, %s, %s, 'Scheduled', %s)
        """, (session['company_id'], staff_id, quote[0], schedule_date, f"Quote Work: {quote[1]}", f"Ref: {quote[1]}"))
...
def service_desk():
    ...
    # Try fetching with smart address handling if needed, but assuming standard for now
    cur.execute("""
        SELECT r.id, p.site_address, r.issue_description, c.name, r.severity, r.status, r.created_at 
...
def create_work_order():
    ...
    try:
        ...
        cur.execute("""
            INSERT INTO jobs (company_id, client_id, staff_id, start_date, description, status) 
...
def staff_list():
    ...
    if request.method == 'POST':
        try:
            cur.execute("""
                INSERT INTO staff (company_id, name, email, phone, role, status) 
...
def fleet_list():
    ...
    cur.execute("""
        SELECT v.id, v.reg_plate, v.make_model, v.status, v.mot_due, v.tax_due, v.insurance_due, v.service_due, v.tracker_url, v.defect_notes, s.name, v.assigned_driver_id, COALESCE(v.daily_cost, 0), COALESCE(s.pay_rate, 0) 
        FROM vehicles v 
...
# --- 12. CLIENT DETAILS (FIXED: START_DATE + DESCRIPTION) ---
...
def view_client(client_id):
    ...
    for row in cur.fetchall():
        ...
        # Address handling (using site_address based on your schema)
        addr_val = p.get('site_address') or p.get('address') or p.get('addr') or 'Unknown'
        prop_id = p.get('id')
        ...
        # Robust Fetch for Jobs on this property
        # Fix: using start_date and description
        ...
        properties.append({
            'id': prop_id, 'addr': addr_val, 
            'postcode': p.get('postcode',''), 'tenant': p.get('tenant_name',''), 
            'tenant_phone': p.get('tenant_phone',''), 'compliance': compliance
        })
    ...
    for r in cur.fetchall():
        # Mapping description to type/notes for the template
...
def add_property(client_id):
    ...
    cur = conn.cursor()
    try:
        # Assuming properties table has standard fields. If schema differs, we might need 'site_address'
        cur.execute("""
            INSERT INTO properties (client_id, site_address, postcode, tenant_name, tenant_phone) 
            VALUES (%s, %s, %s, %s, %s)
...
def upload_compliance():
    ...
    # Need to get property address to link job
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT site_address FROM properties WHERE id = %s", (request.form.get('property_id'),))
    ...
    try:
        # Insert into jobs using start_date and description
        cur.execute("""
            INSERT INTO jobs (company_id, client_id, site_address, description, status, start_date) 
            VALUES (%s, %s, %s, %s, 'Completed', %s)
        """, (session['company_id'], client_id, prop_addr, f"{cert_type} Certificate", comp_date))
        conn.commit()
        flash(f"✅ {cert_type} Uploaded")
    except Exception as e: 
        conn.rollback()
        flash(f"❌ Error: {e}")
    finally: 

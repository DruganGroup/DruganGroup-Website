...
def fleet_list():
    ...
    cur.execute("""
        SELECT v.id, v.reg_plate, v.make_model, v.status, v.mot_due, v.tax_due, v.insurance_due, v.service_due, 
               v.tracker_url, v.defect_notes, s.name, v.assigned_driver_id, COALESCE(v.daily_cost, 0)
        FROM vehicles v LEFT JOIN staff s ON v.assigned_driver_id = s.id WHERE v.company_id = %s ORDER BY v.reg_plate
    """, (comp_id,))
    raw_vehicles = cur.fetchall()
    vehicles = []
    for row in raw_vehicles:
        cur.execute("""
            SELECT s.id, s.name, s.role FROM vehicle_crews vc JOIN staff s ON vc.staff_id = s.id WHERE vc.vehicle_id = %s
        """, (row[0],))
        crew = [{'name': c[1], 'role': c[2]} for c in cur.fetchall()]
